import { updateAvatarnameSchema, updateMetadataSchema } from "@repo/my-types/httpTypes";
...
// route to add users metadata avatar id - tested
...
// route to update user avatarname - tested
userRouter.post("/avatarname", async(req, res) => {
    const parsedData = updateAvatarnameSchema.safeParse(req.body);
    ...
    try{
        ...
        await client.user.update({
            where: {id: req.userId},
            data: {
                avatarname: parsedData.data.avatarname  
            }
        })
        res.json({message: "Metadata updated"})
    }catch(e){
